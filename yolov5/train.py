...
from utils.general import (LOGGER, check_amp, check_dataset, check_file, check_git_status, check_img_size,
                           ...
                           one_cycle, print_args, print_mutation, strip_optimizer, yaml_save, get_yaml_config_file)
...
def parse_opt(known=False):
    ...
    model_dir_default  = (os.environ["SM_MODEL_DIR"] if "SM_MODEL_DIR" in os.environ else "")
    ...
    #parser.add_argument('--sagemaker', action='store_true', help='run it in sagemaker')

    opt = parser.parse_known_args()[0] if known else parser.parse_args()

    opt.project = opt.outputpath

    # def rm_initial_dot_or_sep(dirpath):
    #     for t in [0,1]:
    #         if dirpath.startswith('.') or dirpath.startswith(os.sep):
    #             dirpath = dirpath[1:]
    #     return dirpath 
    
    opt.trainds =  opt.trainds
    opt.valds   = (opt.valds if opt.valds else opt.trainds) # if it is not None
    opt.testds  =  opt.valds

    opt.data = get_yaml_config_file(
        '',
        partitions_dir_dict = {
            'train':opt.trainds,
            'val':opt.valds,
