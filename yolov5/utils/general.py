...
def get_yaml_config_file(
    outputdir,
    partitions_dir_dict = {
        'train':'../datasets/xView/res0.3/training/stride700_wind700/images',
        'val':'../datasets/xView/res0.3/validation/stride700_wind700/images',
        'test':'../datasets/xView/res0.3/test/stride700_wind700/images'
    },
    names = None
):
    """
    Params:
        partitions_dir_dict (str): A dictionary with the three subfolder partitions
    Returns: (str) yaml filename.
    """
    
    dict_yaml = {**partitions_dir_dict,**{
        'path':'',
        'nc':60,
        'names':[
    ...
    if names:
        dict_yaml['nc'] = len(names)
        dict_yaml['names'] = names
    
    filename = os.path.join(outputdir,"tmp.yaml")
    with open(filename,'w') as tmp:
        _ = yaml.safe_dump(dict_yaml, tmp,default_flow_style=None)
    return filename
